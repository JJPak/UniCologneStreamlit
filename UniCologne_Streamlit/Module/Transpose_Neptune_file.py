...
def transpose_oldNeptune_To_Excel (df, element, export_name):
    ...
    for i in range(len(list_pandas)):
        list_pandas[i].columns = list_pandas[i].iloc[16]
        
    # drop the first 20 rows that are not needed of the individual subdataframes
    for i in range(len(list_pandas)):
        list_pandas[i] = list_pandas[i].drop(list_pandas[i].index[0:17]).reset_index(drop = True)
